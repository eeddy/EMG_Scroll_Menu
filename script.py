...
def parse_log_directory(log_dir):
    ...
    for f_path in log_files:
        if ".meta" in f_path or ".txt" not in f_path:
            continue
        f_participant = f_path.split("\\")[-2]

        with open(f_path, "r", encoding="utf-8", errors="ignore") as f:
            lines = f.readlines()

        retval.append([f_participant, lines])
...
def get_round_data(round_lines, num_btn, trial_num, control_scheme, cur_round):
    ...
    for trial_line in round_lines:
        ...
        if "Button Clicked:" in trial_line:
            curr_time_str = trial_line.split("]")[0].replace("[", "")
            curr_time = datetime.strptime(curr_time_str, "%H:%M:%S.%f")
            curr_delta = curr_time - starting_time
            if trial_line != round_lines[-1] and curr_delta.seconds > 1:
                miss_clicks += 1
            continue
    ...
    return [
        ...
        "EMG" if control_scheme == "1" else "CV",  # Control Scheme
        num_btn,  # Num Buttons
        f"{cur_round}",  # Round
...
def get_trial_data(trial_lines, num_btn, trial_num, control_scheme):
    ...
    for i in range(len(trial_lines)):
        ...
        j = 1
        while "Button Clicked:" in trial_lines[i + j]:
            j += 1
        rounds.append(trial_lines[starting_indx:i + j])
        starting_indx = i + j
        if not trial_lines[i + j]:
            continue
        new_goal_btn = int(trial_lines[i + j].split("[DEBUG]")[1].strip().split(",,")[1].split(":")[1].strip())
...
def main():
    ...
    for log_file in logs:
        ...
        if log_file[0] not in worksheets.keys():
            worksheets[log_file[0]] = {}
            worksheets[log_file[0]]["worksheet"] = workbook.add_worksheet(log_file[0])
            worksheets[log_file[0]]["data"] = []
        ...
        for i in range(len(trials)):
            ...
            summary_data[log_file[0]][i+1+len(summary_data[log_file[0]].keys())] = trial_data

        worksheets[log_file[0]]["data"] += log_data

    for key in worksheets.keys():
        worksheets[key]["worksheet"].set_column(f'A:{chr(len(columns) + 64)}', 20)
        worksheets[key]["worksheet"].add_table(
            f'A1:{chr(len(columns) + 64)}{len(worksheets[key]["data"]) + 1}',
            {
                'data': worksheets[key]["data"],
                'columns': columns
            }
        )
